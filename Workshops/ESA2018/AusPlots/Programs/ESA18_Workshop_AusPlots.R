#****************************************************************************************
# Title: ESA18_Workshop_AusPlots.R
...
# =======================================================================================
# Commented out as packages pre-installed for the workshop
# Remove comment symbols ('#') to install the packages

# Accessing and Installing the ausplotsR' Package (plus its Dependencies)
# -----------------------------------------------------------------------
# The package 'ausplotsR' can be installed directly from github using the 'devtools' 
# package, which must have been previously installed. 
...
# Install other packages required for this section of the workshop
# ----------------------------------------------------------------
# A list of Compresenhive R Archive Network (CRAN) mirror URLs can be found  here: 
...
#install.packages(c("ausplotsR", "vegan", "goeveg", "maps", "maptools", "mapdata", 
#                    "sp", ggplot2", "gridExtra", "ggspatial", "dendextend"), 
#				  repos=my.repos)
...
# SET UP
# =======================================================================================
# Load the libraries that we will need for the workshop
...
# Up to 8 different types of data can be obtained by setting the corresponding arguments 
# to TRUE/FALSE:
...
# In addition AusPlot data can be subset via the 'get_ausplots' function arguments in two 
# other ways:
...
# The R object resulting from calling 'get_ausplots' is a list of data frames containing 
# the requested AusPlots data. The list includes a data frame for each type of data requested 
# (i.e. up to 8 data frames: 'site_info', 'structural_summaries',...). These data frames are 
# interrelated and they all have a common variable, 'site_location_name'. In each data frame 
# the columns correspond to the variables supplied for each type of data and the number of 
# rows (directly or indirectly) depends on the sites retrieved (e.g. via 'my.Plot_IDs' or 
# 'bounding_box').
...
# data. However, the 'deep' structure of the data and interrelation of the data frames 
# (via the common 'ste_location_name' variable) can make manipulating the data a bit more 
# daunting. 
...
# ---------------------------
# Extract Sites Names
AP.200Locs.FC.locs = sub("\\-.*", "", AP.200Locs.FC$Plot)
...
dim(AP.200Locs.FC.Resampled.Locs) # 80 (= 40 * 2)

# Add Year (Started) Sampling of Site-Visit Pair
# ----------------------------------------------
# Need to specify 'AP.BioregTop5.l$site.info[,c("Plot","visit_start_date")]' to avoid duplicate columns
AP.200Locs.FC.Resampled.Locs  = merge(AP.200Locs.FC.Resampled.Locs, AP.BioregTop5.l$site.info[,c("Plot","visit_start_date")], 
														by="Plot")[,c(names(AP.200Locs.FC.Resampled.Locs),                                                    "visit_start_date")]
...
AP.200Locs.FC.Resampled.Locs$Plot.Yr = paste( AP.200Locs.FC.Resampled.Locs$Plot,  
											  substr(AP.200Locs.FC.Resampled.Locs$visit_start_date,1,4),
											  sep="." )
head(AP.200Locs.FC.Resampled.Locs$Plot.Yr)

# Plot Pies for the first 5 Resampled Sites -out of 40- (i.e. 10 Site-Visit pairs)
# --------------------------------------------------------------------------------
# Order dataframe to Plot Site-Visit pairs in the appropriate order
AP.200Locs.FC.Resampled.Locs = AP.200Locs.FC.Resampled.Locs[order(AP.200Locs.FC.Resampled.Locs$Plot),]
...
for (site.visit.cnt in 1:10) {
	...
	pie( x=as.numeric(AP.200Locs.FC.Resampled.Locs[site.visit.cnt,2:5]), 
		 ...
		 main=as.character(AP.200Locs.FC.Resampled.Locs[site.visit.cnt,"Plot.Yr"]) )
