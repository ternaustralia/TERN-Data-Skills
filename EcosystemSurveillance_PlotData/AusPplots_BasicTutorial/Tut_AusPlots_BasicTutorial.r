...
# corresponding to the concept by we would like to filter the data. In some occasions we would 
# sub-setting a single data frame (i.e. type of variables) is all what we need. Variables in the 
# `site.info` data frame contain information that affect all other data frames; so typically after 
# sub-setting the contents of the variable of interests in the `site.info` data frame, we will also 
# subset the remaining datasets using one of the common variables among all data frames. Common 
# variables among datasets include `site_location_name`, `site_location_visit_id`, and `site_unique`.  
# Commonly `site_unique` is the best option to ‘connect’ ausplots data frames, as it is the most 
# specific variable representing a single visit to a particular site.
...
# those in the National Vegetation Information System (NVIS). 
...
3 understorey the values returned by both functions can differ. For general cover purposes, using 
