...
All entities in the information model have:
...
-   See the [TERN Ontology](https://linkeddata.tern.org.au/information-models/overview) for more information and [the conceptual overview of the model](https://linkeddata.tern.org.au/information-models/tern-ontology/conceptual-information-model).
...
-   data folder that contains:
    ...
    -   Site and site attributes file
        ...
        -   If other sampling units are used then a file for the sampling unit is included. A transect file is included when the sampling takes place in a transect which may be part of a site.
...
Note: you will need to have an API "key" to use this function. Further information regarding using the API and how to obtain an API key can be found here[TERN API](https://ecoimages.tern.org.au/api/v1.0/ui). Alternatively, you can visit [TERN Account](https://account.tern.org.au) to generate your API key. 

# Data formatting - Long to wide format

The basic columns of the longer format tables are:

-   `featureType` is a human readable name for the feature of interest on which observation was made. A complete list of all feature types can be found [here](https://linkeddata.tern.org.au/viewer/tern/id/http://linked.data.gov.au/def/tern-cv/68af3d25-c801-4089-afff-cf701e2bd61d)
...
In general, the conversion from longer to wider format should be achieved by using `featureType` and `featureId` as unique identifiers, the `parametersName` are the name of parameters (observable property) that are observed and are part of the FOI and `result` is the outcome for a given observable property. We will now work though a basic example of formatting and visualising biomass data downloaded from EcoPlots
