#****************************************************************************************
# Title: ESA18_Workshop_AusPlots_P2.R
# Author: Bernardo Blanco-Martin, TERN
# Date: 6/10/2018
#****************************************************************************************
# In Part 2 of the AusCover section of the workshop, we will investigate the Changes in 
# Normalized Difference Vegetation Index (NDVI). In particular we will cover the following 
# points:

# NORMALIZED DIFFERENCE VEGETATION INDEX (NDVI) CHANGE ANALYSIS:
#  * Explore an Area where NDVI Change might have occurred
#  * Conduct Tasks in the 'Introductory Steps' part (i.e. Download, Read, Subset, and 
# Combine the Landsat Surface Reflectance Data) by:
...
# In this section we will explore the change in NDVI in an area west of Tara where Coal 
# Seam Gas development has been happening in recent years. NDVI can be used to estimate 
# the density of green vegetation on a patch of land. It is derived from remote sensing 
# data, typically a space platform. Chlorophyll, the pigment in plant leaves, strongly 
# absorbs red and blue light. Leaves, on the other hand, strongly reflect near-infrared 
# light (NIR) and green light (this is why vegetation looks green to our eyes). NDVI 
# takes advantage of this difference in behaviour for different light wavelengths by 
# green vegetation, combining measurements at different wavelengths in  this formula: 
# NDVI = (NIR-Red)/(NIR+Red). 
# NDVI ranges from -1 to 1. With different values indicating different situations:
...
# * Very low positive to slightly negative values: Free standing water (e.g. oceans, seas, 
# lakes, rivers), which have rather low reflectance in both spectral bands.
# * Values close to '-1': likely to represent atmospheric disturbance or some specific 
# materials. 

# Generally, to investigate NDVI change over time an atmospheric correction needs to be 
# performed. The data that we use in this workshop has already been corrected to reduce 
# contamination by cloud and other problems.


# ****************************************************************************************
# Preparation: Getting ready
# ****************************************************************************************
# If you need to install the packages required for this section of the workshop, you can 
# use code below. A list of Compresenhive R Archive Network (CRAN) mirror URLs can be 
# found here: https://cran.r-project.org/mirrors.html.
...
#install.packages(c("dplyr", "reshape2", "stringr", "RColorBrewer", "ggplot2", "gridExtra", 
#                   "RStoolbox", "sp", "rgdal", "raster", "rasterVis", "ggmap"), 
#                 repos=my.repos)


# We start by loading the required libraries, setting up a working directory, and cleaning 
# up memory. 
...
# Explore an Area where NDVI Change might have occurred
# ****************************************************************************************
# We import and display a Stamen map to explore the area that we will be working with 
# (West of Tara). The first attempt, using the extent of this area, leads to too detailed 
# map. We add a plot where we zoom out by 2 degrees and now we can identify a nearby 
# location (Dalby).
...
# **********************************************************************************************
# This function includes many of the steps in the INTRODUCTORY STEPS section. Specifically 
# the function perform these tasks: 
...
# There are a number of `print` commands within the function 'commented out'. These commands 
# are not required for the correct functioning of the function. However, they have been included 
# ecause they might make it easier to understand what the function is doing. To investigate further 
# what the function is doing at each step simply remove the comment (`#`) symbol in front of the 
# relevant 'print' command.
...
#******************************************************************************************************************************************
# Use our new function to get the Landsat Surface Reflectance datasets for Autumn 2007 and Autumn 2017, then subset the required bands and 
# finally create and return a `RasterBrick` with the resulting `RasterLayers`. The RasterBricks for these periods are then plotted together. 
...
# * 'Raster Algebra' (also known s 'Raster Maths'): Directly combining raster objects in a 
# formula with the required calculations. We can use this formula directly, or place it in 
# a function and call this function. Typically used for small rasters and simple 
# calculations (not recommended otherwise).
# * 'Higher Level Functions': First we place the formula with the required raster 
# calculations in a function and then we use a Higher Level Function to invoke this 
# function on relevant raster objects. This is more efficient (faster) option, which should 
# be used for larger rasters and/or complex formulas. The Higher Level Function used depends 
# on the number of raster object involved in the calculations:
...
# To compute NDVI values we first create a formula with the required maths (for details 
# on the NDVI see text at the beginning of this section (NORMALIZED DIFFERENCE VEGETATION INDEX 
# (NDVI) CHANGE ANALYSIS). We actually create two formulas, the first one passing the RasterBrick 
# to be used with 'calc' and the second one passing individual layers to be used with 'overlay'. 
# Then we calculate this index in different ways: 
...
# Sometimes 'calc' and 'overlay' don't work; we show a trick (generating an empty raster with 
# the right features and initialising it before assigning the values we need for our work to it) 
# to help it work for us. The we compare the results of these 4 approaches to prove that the all 
# produce the same results.

# NOTE that in these datasets the value 32767 is used as a no-data code, so we will replace 
# this value by NA
...
# To plot the NDVI's of both year we first create a RasterBrick with both RasterLayers. 
# We could plot both RasterLayers side to side but by default this would produce smaller 
# plots as the plots would include density plots on their margins, which are not strictly 
# comparable. They would also include two scales; plotting a RasterBrick a single scale is 
# shown.
...
# ****************************************************************************************
# Now we explore the change in NDVI in the 10 year period (2007 to 2017). First, we 
# compute the change in NDVI and then we visualise it.
...
# Differences in NDVI can be due to many factors. Typically, the largest difference is due 
# to climatic effects. Here the overall larger NDVIs in 2017 than in 2007 are due to a much 
# wetter start of the year in 2017 than in in 2007. However, there are also specific areas 
# of change. There are more watering points (indicated by very low NDVI values) and there 
# is more coal seam gas infrastructure. 

# To make changes in NDVI slightly easier to interpret we normalise NDVI values (i.e. 
# (NDVI - mean(NDVI)) / std(NDVI) ), so that 0 represent the mean NDVI and changes are in 
# standard deviations of NDVI. To compute the summary statics values of a raster object 
# (e.g. mean and standard deviation in our case) we need to use the `cellStats` function 
# in the `raster` package. See *TERN's DSDP Tutorial 'Using Raster Data in R'* for details.

# As when we calculated NDVI, NDVI changes (both in absolute NDIV values and in stdev. units) 
# can be computed using:
# * 'Raster Algebra' (= 'Raster Maths'): either directly using a formula, or placing this 
# formula in a function and calling the function.
# * 'Higher Level Functions', such as `calc` and `overlay`.

# For normalising the NDVI values in a raster we can use the function `normImage` in the 
# package `RStoolbox`. We could incorporate this function both in 'Raster Algebra' 
# formula/function and in the function called by the 'Higher Level Function'. Here we will 
# use 'Raster Algebra' both specifying the complete formula and taking advantage of the 
# function `normImage`.
...
#  	 * Boxplots: Using the function `bwplot` from the package `rasterVis`. This function 
# creates violin plots, a boxplot with a kernel density plot.
#    * Matrix of scatterplots with a fitted loess to investigate the relationship between 
# both years.
#  * The NDVI values of individual years and their change using Density plots: We will build 
# the density plots in two ways: 
#    * Using the function `densityplot` from the package `rasterVis`: Simple easy plots.
#    * Using the function `ggplot` from the package `ggplot2`:  This looks prettier and is 
# more flexible, but requires more work.
#  * The NDVI change between years by Plotting a RasterLayer of this change (in both scales: 
# raw NDVIs and normalised NDVIs).

# We will start by creating a RasterBrick object to pass to many of the plotting functions 
# (e.g. the functions in the package `rasterVis`). 
...
#-----------------------------------------------------------------------------------------
# They have very different scales (raw NDVI and normalised NDVI values)
...
# Create Raster Plots with Density Plots
# ......................................
wofTara.NormNDVIDiff.p = levelplot( wofTara.NDVI.NormDiff.2017to2007.rL.m2, at=NormNDVI.Diff.breaks, 
                                    col.regions=NormNDVI.Diff.cols, main="Normalised NDVI Change (2017 - 2007)")
...
#*****************************************************************************************
# Aside from areas of where infrastructure has been put in and a small patch of clearing in
# the south-east corner, there is an area of noticeable greening up in the north-east.

# To examine what is happening here overtime we can plot a time series of the green 
# fraction. The green fraction is one of the bands of the fractional cover product. This is 
# another AusCover product hosted by TERN.

# Then we fit a trend line to the green data over the period of interest.

# The server hosts time series stacks of cover fractions in virtual files that make pulling 
# out time series a breeze. However, individual bands could also be iterated without too 
# many problems.
...
# The plots for the Satellite (Surface Reflectance) images are `ggplot2` plots. The 
# Normalised NDVI Change images is a `lattice` plot. The procedure to add a polygon box to 
# ggplot2 & lattice plots differ. We plot both types of plots to show how to add a polygon 
# in each case.
...
#=========================================================================================
# We start by downloading the `vrt` file containing the green cover fraction. VRT is format 
# (Virtual Format) driver for GDAL (Geospatial Data Abstraction Library) that allows building 
# virtual GDAL datasets from other GDAL datasets. '.vrt' files contain 'VRT' descriptions of 
# datasets typically saved in XML format. For further details see 'GDAL Virtual Format Tutorial' 
# by GDAL organization [here](https://www.gdal.org/gdal_vrttut.html) 
 
# NOTE: That the name of the vrt file might have changed since this document was written, as 
# files get updated (i.e. they grow) with new data.
...
#=========================================================================================
# The green cover fraction vrt file currently contains 122 bands/layers. Four new bands 
# are created and added each year. Bands 77-120 correspond to the period of interest, 
# 2007-2017 (one band has already been added for 2018). You can check this (and the file 
# structure) opening it in text editor such as Notepad.

# We create a new function to Load, Subset (i.e. Crop), Recode (the no-data values) and 
# Combine the bands of interest. This function is partially similar to the function 
# 'get.B3B4B5.rB.f' that we created above. Then we call this function to create a RasterBrick  
# containing the Green Cover Fraction bands of interest (bands 77-120 for the period 2007-2017) 
# subsetted to the extent of the Greening Area we want to explore. 
...
#=========================================================================================
# First we compute a vector with the mean Green Cover Fraction (GCF) in the Greening Area 
# (GA) for each raster layers. The raster layer values are in scale 0-255, so we re-scale 
# the mean value to scale of 0-100 to represent the Green Cover Fraction in Percentage. 

# Then we create a vector with the Times (Years and Quarters) for each of these raster 
# layers. The function that we used to create the Raster Brick with all the relevant data 
# computed the date for each layer and assigned it as part of the corresponding band (i.e. 
# layer) name. Here, we take advantage of this and extract the Times from the bands names. 
# We could also have calculated it in a similar fashion as it was done within the function.
...
# (1) a linear model fit, (2) a linear model fit with 2 degree polynomial (using the function 
# 'poly'), and (3) a locally weighted regression (loess). For prettier and more flexible graphs 
#  use the 'ggplot' function from the ggplot2` package. 
# * Violin Plots: Violin plots combine Boxplots and Density plots (see above). We use the 
# nction 'bwplot' from the package `rasterVis` to create this type of plot.  
