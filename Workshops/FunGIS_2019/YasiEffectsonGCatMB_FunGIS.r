...
# This document contains background information and a step-by-step guide (including R code and results) for TERN's 
# (Terrestrial Ecology Research Network) workshop at the FunGIS 2019 Conference (Cairns, 16th-17th May 2019). In this 
# workshop we will explore the effect of Cyclone Yasi on the green vegetation fraction in Djiru National Park at the 
# backdrop of Mission Beach. In the process we will explain and familiarise with the use of remote sensing data 
# (particularly TERN's data) in R. An expanded version (additional explanations and some extra code) of this document 
# can be found in TERN's DSDP "Investigation of the Effects of Cyclone Yasi on Green Cover Fraction at Mission Beach 
# using TERN's Remote Sensing Data" tutorial. Detailed information on how to use raster data in R can be found in TERN's 
# DSDP "Using Raster Data in R" tutorial. Additional examples of the usage of TERN's Remote Sensing data can be found in 
# TERN's DSDP "Green Vegetation Temporal Dynamics: Monitoring and Analysis using NDVI and Fractional Cover TERN rasters 
# (Parts 1 & 2)" tutorials.  
...
# `stackApply`: Computes summary type layers for subsets of a RasterStack or RasterBrick. To do so it uses a summary function applied on 
# subsets of layers of a RasterStack or RasterBrick object. The summary function must return a single value.
...
# Maths' when rasters are large and/or calculations complex and/or rasters stored in separate individual files. The raster objects used as 
# inputs can be single-layered or multi-layered rasters (with the same number of layers). Both case produce similar results. In the multi-
# layered rasters case the function is applied to each pair of layers.
...
# functions `bwplot` and `splom` in the package `rasterVis`. An alernative method using the function `ggplot` in the package 
# `ggplot2` produces nicer graphs but requires more elaborated coding, so it is presented in the expanded version of  this 
# Workshop tutorial (*"Investigation of the Effects of Cyclone Yasi on Green Cover Fraction at Mission Beach using TERN's 
# Remote Sensing Data"* tutorial). To visualise the cell values of the difference rasters we use the function `levelplot` 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# See the expanded version of this workshop tutorial ("Investigation of the Effects of Cyclone Yasi on Green Cover 
# Fraction at Mission Beach using TERN's Remote Sensing Data" tutorial).	  
...
# Create individual plots of SPGC rasters for both years (previously both years graphed in a single plot)
SA.SGGC.2011q3.p = levelplot(SGGC.StudyArea.2010q3.rl, at=SGGC.breaks, col.regions=SGGC.cols, main="Seasonal Persistent Green Cover (2010-Winter)")
SA.SGGC.2010q3.p = levelplot(SGGC.StudyArea.2011q3.rl, at=SGGC.breaks, col.regions=SGGC.cols, main="Seasonal Persistent Green Cover (2011-Winter)")
## Plot Raw Values (top row) + Raw & Standardised Change (bottom row)
...
# If argument `bylayer` is TRUE, the individual layers of multi-layer rasters (RasterStack and RasterBrick objects) are 
