...
AP.data = get_ausplots(basal.wedge=TRUE, structural_summaries=TRUE, standardised_name_search="Eucalyptus") 
...
AP.200Locs.FC.locs.cnt = plyr::count(AP.200Locs.FC.locs)
...
# The (currently) most common codes correspond to: MDD (Murry Darling Depression),
# SSD (Simpson Strzelecki Dunefields), GFU (Gulf Fall and Uplands), STP (Stony
# Plains), PIL (Pilbara), FLB (Flinders Lofty Block).
# For example, here PIL is commented out 'cos when writting the current version of
# this tutorial it wasn't one of the 5 most sampled bioregions. However, in the
# previous version of this tutorial PIL was included, and FLB wasn't among the 5
# most sampled bioregions.
...
#levels(AP.BRTop5.GrowthFormBYSites$bioregion.col.f)[levels(AP.BRTop5.GrowthFormBY#Sites$bioregion.col.f) == "PIL"] = "red"
...
# Plot Dendrogram
par(mfrow=c(1,1))
dend2 <- color_unique_labels(AP.BRTop5.GFBYSites.dend)
plot(dend2,main="Dendogram of the Sites-Visits Clustered by Growth Forms, with leaves coloured by Bioregion")
...
```{r, message=FALSE, warning=FALSE, error=FALSE, fig.height = 8, fig.width = 7}
