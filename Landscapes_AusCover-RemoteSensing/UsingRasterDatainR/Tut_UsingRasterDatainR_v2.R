#************************************************************************************************************************
#
#  File: Tut_UsingRasterDatainR_v2.r
#
#  Author: Bernardo Blanco-Martin
#  Date: 27/03/2019 (v1: 06/09/2019)
#  Description: Thorough introduction to working with Raster in R.
#
#************************************************************************************************************************
...
# Raster are also sometimes referred to as `grids` and cells as `pixels`. Rasters represent spatial data as a continuous. 
# Raster are therefore in sharp contrast with the other main structure used to store and manipulate geographical data, 
# `vectors`. Vectors represent discrete (i.e. object based) spatial data, such as points, lines, polygons.

# The fundamental R package for working with spatial data in raster format is `raster`, originally developed by Robert J. 
# Hijmans. The `raster` package provides classes and functions to create, read, manipulate and write raster data. In this 
# tutorial we will describe and experiment with many of these classes and functions. A notable feature of this package is 
# that it can work with very large spatial datasets that cannot be loaded into computer memory. Functions process these 
# large datasets in chunks, without attempting to load all values into memory at once. `raster` has revolutionised the 
# manipulation, geo-processing and analysis of raster data in R. More information on `raster` can be found in the package 
# vignette [here]( https://cran.r-project.org/web/packages/raster/vignettes/Raster.pdf).
   
# The `raster` package relies on the `rgdal` package to read, write, and geo-process raster data. `rgdal` provides bindings
# to the Geospatial Data Abstraction Library ('GDAL') and access to projection/transformation operations from the 'PROJ.4' 
# library, both external to `rgdal`. It is also possible to call GDAL functionalities directly from R and the command line 
# in a terminal (e.g. using the function `system()` in R). Many spatial software packages also use GDAL to read/write 
# gridded spatial data (e.g. ArcGIS, QGIS, GRASS, etc.). More information on the GDAL can be found on [this link]
# (https://www.gdal.org/) and more information in PROJ.4 can be found in [this link](https://proj4.org).
...
# * `RasterStack`: Object containing a multi-layer (band) raster. It can ''virtually'' connect several raster objects written
#  to different files or in memory and/or a few layers in a single file.
# * `RasterBrick`: Object containing a multi-layer (band) raster. It is a truly multi-layered object. It can only be linked 
# to a single multi-layer file (i.e. all data must be stored in a single file on disk) or is in itself a multi-layer object 
# with data loaded in memory. Typical examples of multi-layered raster files are multi-band satellite images and rasters 
# containing time series (e.g. each layer contains values for a different day or month).

# RasterStack and RasterBrick objects are quite similar. However, RasterBricks have a shorter processing time than RasterStacks 
# and RasterStacks are more flexible (e.g. with RasterStacks pixel-based calculations on separate raster layers can be performed).
   
# In both multi-layered object classes, individual layers must have the same spatial extent and resolution. That is, individual 
# layers must represent the same locations with the same level of detail.
 
# In all 3 classes the data can be loaded in memory or on disk depending on the size of the grid(s). Raster objects are typically 
# created from files, but even RasterBrick objects can exist entirely in memory.
...
# * `stack`: Creates a RasterStack object from RasterLayer (i.e. individual layers) objects or from a muliti-layer (band) 
# file. They can also be created from a SpatialPixelsDataFrame object or SpatialGridDataFrame object. Individual raster 
# layers must have the same spatial extent and resolution.
# * `brick`: Ceates a RasterBrick object from RasterLayer (i.e. individual layers) objects, a RasterStack, or from a 
# muliti-layer (band) file. They can also be created from SpatialPixels*, SpatialGrid*, and Extent objects. Individual 
# raster layers must have the same spatial extent and resolution.

# First we create a frame that will contain the Raster* object. This object will have defined: Dimensions, Spatial Extent, 
# Resolution, and a Coordinate Reference System (CRS)
...
# `addLayer`: Adds one or more layers to a Raster* object. The object returned is a RasterStack (unless nothing to add or
#  drop was provided, in which case the original object is returned).
...
# `Subset`: In addition of using the function `raster` to extracts a layer or subset of layers from a RasterStack or 
# RasterBrick object the function `subset` can also be used for to achieve tis task. The argument `subset` is an integer 
# or character indicating the layers to extract. If a single layer is selected and the argument `drop`:
# * `drop=TRUE`: The returned object would be of class RasterLayer.
# * `drop=FALSE``: The returned object would be the same as that in the original raster object (i.e. RasterStack or 
# RasterBrick).
...
# Supported formats include (in parenthesis is the file extension for the format). The last 3 formats cannot store Multi-
# band rasters. See help on 'writeRaster' and 'writeFormat' for further details:
...
# If argument `prj` is TRUE, the CRS is written to a '.prj' file. This can be useful when writing to a file type that does 
# not store the crs, such as ASCII files. 

# For multi-layer rasters (RasterStack and RasterBrick objects), individual layers can be stored in the same file or in 
# separate files. When argument `bylayer` is TRUE, layers are saved to separate files. In this case, to name the separate 
# files the user provide naming directions using the argument 'suffix': 
...
# Loading rasters is done with the same functions that are used to create them from data (i.e. `raster`, `stack`, and 
# `brick`).
...
# Dichotomy: A RasterLayer can be accessed both as a Matrix (via cell row and column numbers) and as a Vector (via cell 
# number).
# * Conceptually: Easier to think of a RasterLayer as a matrix.
# * In pratice: Commonly treated as a vector, constituted by the RasterLayer cells from top-left corner to bottom-right
# ordered by row. 

# Except `dim` and `names`, which is not really Helper Function, all functions return the same results for uni-layer and
#  multi-layer objects (compose of layers of the same characteristics than the single layer).
...
# In this section we will examine the contents (i.e. values) of each cell in the raster. The methods to summarize this 
...
# For multi-layer rasters the procedures are analogous to those obtained for single-layers rasters, but results include
#  the relevant values for all the layers in the object. 

# RasterStack and RasterBrick objects would return the same results if their contents are equal (e.g. s2 & b2 return the
#  same results)
...
# Rasters can be visualised using the function `plot` in the package `raster`. Even better visualisations can be easily
#  achieved with the function `levelplot` in the package `rasterVis`.
...
# * The whole raster layer. `cellStats`, `freq`, and `crosstab` are part of the `raster` package. `summary` is part of the 
#   `base` package, but it includes a method for Raster* objects. The three functions from the raster package can be used
#   for rasters with multiple-layers. However, `summary` can only be used with RasterLayer objects. We can produce:
#   * Summary statistics for a single layer: The function `cellStats` provides a single summary statistics (e.g. max, mean, 
#     min, sd, sum). Some of these summary statistics will fail with very large Raster* objects. For Raster* objects the 
#     function `summary` returns the: minimum, 1st quartile, median, 3rd quartile, maximum, and the number of cells containing
#     missing data. 
...
# * Portions of a raster layer using zonal statistics, via the function `zonal`. This function can also be used with multiple
#   -layers.
# * The neighbourhood of the cells using a moving window, via the function 'focal'. `focal` uses a the moving window to calculate
#  the values for the neighbourhood of the focal cell. The moving window shape and values are defined using a matrix of weights 
#  with a function. In image processing moving windows (generally a moving average) are used to smooth a data set; that is, 
#  to attempt to remove the noise and/or fine-scale structures capturing the main patters in the data. `focal' currently only 
#  works for (single layer) RasterLayer objects. Its output is a new raster. Peculiarities of this function: 
...
#    * Default weight function is 'sum': It is computationally more efficient to adjust the weights-matrix, than to use some other 
#      summary functions through the 'fun' argument (i.e. can compute mean using fractions, see below).
...
# The `raster` package provides many functions to visualy display and summarise the data in `Raster*` objects. The 
# `rasterVis` package complements the `raster` package, providing additional methods for enhanced visualization and
# interaction. 

# Some of the functions in this packages can only be used in single-layer rasters, others can only be used in muti-layered 
# rasters, and the remaining functions can be used for both single-layer and multi-layer rasters. In particular, the 
# visualisation functions that we explore below can be applied the the following raster data types: 
...
# * For a uni-layer (RasterLayer) input, the function  is typically a function that can take a single vector as input and 
# return a vector of values of the same length (e.g. sqrt). 
# * For a multi-layer (RasterStack or RasterBrick) input, the function should operate on a vector of values, with one vector
#  for each cell.
...
# * When input is a RasterStack or RasterBrick and the Function a Summary type Function (i.e. returns a single value), it 
#   returns a RasterLayer.
# * When input is a RasterStack or RasterBrick and the Function returns more than one number (e.g. fun=quantile), it returns
#   a RasterBrick.

# In many cases, what can be achieved with 'calc', can also be accomplished with a more intuitive 'raster-algebra' notation. 
# For example, the log10 of the cells in a raster can be computed as log10(r), which is simpler than using calc with this 
# function. However, `calc` should be faster when using large datasets and/or complex formulas. 
...
# To do so it uses a summary function applied on subsets of layers of a RasterStack or RasterBrick object.These subsets are 
# defined by vector indices. If the number of indeces in the vector is smaller than the number of layers the indices are 
# recycled.
# The summary function must return a single value; thus, the number of layers in the output Raster* equals then number of 
# unique values in the indices vector. If the output raster contains a single layer it would be a RasterLayer object. 
# If it contains multiple layers it would be a RasterBrick object.
...
# `overlay`: It is typically used with multiple Raster* objects, but it can also be used with a single `Raster*` object. 
# However, it is often simpler to use `calc` (or "Raster Algebra") for single Raster* objects. See next section for its 
# application to multi-layered raster objects.
...
# `ovelay`: It is more efficient that 'Raster Maths' when rasters are large and/or calculations complex and/or rasters 
# stored in separate individual files.
...
# The raster objects used as inputs can be single-layered or multi-layered rasters (with the same number of layers). 
# Both cases produce similar results. In the multi-layered rasters case the function is applied to each pair of layers.
...
# `mask()`: It creates a new Raster* object that has the same values as x, except for the cells that are NA (or other 
# maskvalue) in a 'mask'. These cells become NA (or other updatevalue). The mask can be:  A Raster* object of the same 
# extent and resolution, or a Spatial* object (e.g. SpatialPolygons) in which case all cells that are not covered by 
# the Spatial object are set to updatevalue. You can use `inverse=TRUE` to set the cells that are not NA (or other 
# maskvalue) in the mask, or not covered by the Spatial* object, to NA (or other updatevalue). 
...
# `cover()`: Combines layers by taking the values of the first layer except where these are NA. For Raster* objects it 
# replaces NA values in the first Raster object (x) with the values of the second (y), and so forth for additional Rasters.
#  If x has multiple layers, the subsequent Raster objects should have the same number of layers, or have a single layer 
# only (which will be recycled). 
...
# `crop()`: The most commonly used function to modify the extent of a raster object. It returns a geographic subset of an
#  object as specified by an Extent object (or any object from which an extent object can be extracted/created). The 
# Extent object represent our region of interest. When the object to crop is a Raster* object, the Extent is aligned to 
# this object. Areas included in the Extent object, but outside the extent of the object to crop are ignored.

# `drawExtent`: Allows to subset a raster object by clicking on two pionts of a plot (raster). Currently, it returns an 
# error when used in Rmarkdown files (used to prepare this tutorial). However, you can give it a try by using the code 
# below. Simply, remove the comment symbol (`#`) in front of the commands containing `drawExtent` or the raster object 
# rb2.crop3. 
...
# `merge()`: Merges 2 or more Raster* objects to create a single a new Raster object with a larger (or equal) spatial 
# extent. 

# The Raster objects must have the same origin and resolution (so their cells neatly fit into a single larger raster). If
#  this is not the case, first adjust one of the Raster objects with use (dis)aggregate or resample (see below).

# If objects overlap, the values get priority in the same order as the arguments (i.e. the values of the Raster object 
# that is first in the sequence of arguments will be retained).
...
# Trim (shrink) a Raster* object by removing outer rows and columns that contain the same value (e.g. NA). The multi-
# layer case is analogous to the single-layer case.
...
# Extend returns a Raster* object with a larger spatial extent, by adding new rows and/or columns with a give value (e.g. 
# NA). The output Raster object has the outer minimum and maximum coordinates of the input Raster and Extent arguments. 
...
# * Providing a numeric vector with the number of rows and columns to be added at each side. This vector can have length 2 
# (i.e.rows and columns) or length 1 if the number of rows and columns is equal.
...
# `aggregate`: Aggregates a Raster* object to create a new RasterLayer or RasterBrick with a lower resolution (i.e. fewer 
# larger cells). Aggregation groups rectangular areas to create this larger cells. The value for the resulting cells is 
# computed with a user-specified function (e.g. `mean`). Different aggregation functions can be used in the x and y direction.
...
# `disaggregate`: Does the opposite than 'aggregate'. It disaggregates a RasterLayer to create a new RasterLayer with a 
# higher resolution (i.e. more smaller cells). The values in the new RasterLayer are the same as in the larger original 
# cells; unless you specify method="bilinear", in which case values are locally interpolated (using the resample function). 
...
# `resample`: Transfers values between Raster* objects with different origin and/or resolution. If Raster* objects also 
# have different projections (i.e. coordinate reference system), `projectRaster` can be used to re-project one of the 
# objects (see next section).

# Before using 'resample', the use of other raster functions should be considered (i.e. `aggregate`, `disaggregate`, 
# `crop`, `extend`, `merge`) as they might be more appropriate to achieve the required task.
...
# `projection`: Get or set the Cordinate Reference System (CRS) of a Raster* object. It Obtains or Adds information for 
# a Raster object, but doesn't change the Projection.
...
# * Providing a Raster* object with the properties that the input data should be projected to: Has more control over the
#  transformation (e.g. to assure that the new object lines up with other datasets).
...
# The Extent, Resolution, and Projection can be set to be the same in all rasters using the `spatial_sync_raster` function
# from the `spatial.tools` package.
...
# The raster function `interpolate` returns a raster object with predictions, and requires (at least): a raster object and 
# a model object. The models uses the cell coordinates as predictor variables and the cell values as the dependent variable.
#  Here we will use a Thin Plate Spline Model to create (i.e. fit) the model object. Multiple models can be fitted using:
...
# As with the function `interpolate`, the function `predict` also returns a raster object with spatial predictions and has 
# as inputs a raster object and a model object. However, these are different than in the function 'interpolate':
# * Raster object: is typically a multi-layer raster (i.e. RasterStack or RasterBrick)
# * Model object: is a model of any class that has a 'predict' method, including: glm, gam, randomForest, and is fitted to
#   raster data.

# This approach of predicting from a fitted model to raster data is commonly used in remote sensing (e.g. for the 
# classification of satellite images) and in ecology (e.g. for species distribution modelling).

# For further informatin see help pages for [the predict function in the package raster](http://127.0.0.1:25588/library/raster/html/predict.html)
# and the package [dismo](http://127.0.0.1:25588/library/dismo/html/dismo-package.html).
...
# Attention must be paid to the raster projection as it can noticiably affect the results (small areas often use a UTM 
# projection, while large areas/world oftne use '+proj=longlat'). Illustrations of the different results obtained using 
# different projections can be seen in the examples for the functions `pointDistance` and `gridDistance` below.
...
# * `gdistance`: Computes Distances and Routes on Geographical Grids. It includes advanced distances, such as: cost distance 
#   and resistance distance. See package vignette [here](https://cran.r-project.org/web/packages/gdistance/vignettes/gdistance1.pdf). 
#   A tutorial on how to calculate Cummulative Distances usign this package can be found [here]
#   (http://personal.colby.edu/personal/m/mgimond/Spatial/Distance_rook_vs_queen_vs_knight.html)
# * `geosphere`: Uses spherical trigonometry for geographic applications. It computes great-circle distances (and related 
#    measures)for angular (i.e. spherical longitude/latitude coordinates) locations. See package vignette [here]
...
# `pointDistance`: Computes the shortest geographic distance between two (set of) points. When sets of points are given,
#  if both sets have:
#  * Equal number of points: The distance between each point and the corresponding point in the other set is calculated, 
#    except if the argument allpairs is TRUE (i.e. `allpairs=TRUE`).
...
# * TRUE: Coordinates should be in degrees. The distance is calculated on the World Geodetic System (WGS) ellipsoid.
#   The units of the calculated distances are meters. 
# * FALSE: Coordinates represent Euclidian space (e.g. units of meters). The distance is calculate d on a plane. 
#   The units of the calculated distances are map units (often also meters). 
...
# `gridDistance`: Computes the distance from a cell or set of cells to cells in a RasterLayer. The distances are 
#  computed by summing the local distances between the center of the cells. The path can follow any of the 8 
#  neighboring cells (i.e. the 'queen' case, which is currently the only one implemented). It can be specified that 
# certain types of cells (i.e. with a given value) cannot be traversed by the path. For example, water bodies can be
#  excluded by providing the cell value representing them. 

# If the RasterLayer to be processed is big, it will be processed in chunks. This may lead to errors when of complex
#  objects spread over different chunks (e.g. meandering rivers). Varying the chunk size might help solving these
#  issues (see function `setOptions`).
...
# To compute measures of autocorrelation in a RasterLayer, two autocorrelation measures are available: Moran's I and 
# Geary's C. Both can be computed: Globally (returns a number) or Locally (returns a RasterLayer object). They use 
# weights defined by a matrix, as in 'focal' (see above 'Summarizing values in a "moving window"'). The Weights Shape 
# and Size can be changed. The default is: 'w=matrix(c(1,1,1,1,0,1,1,1,1), 3,3)'; i.e. a 3x3 matrix.
