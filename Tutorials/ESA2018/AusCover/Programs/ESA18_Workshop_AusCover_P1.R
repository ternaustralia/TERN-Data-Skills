#****************************************************************************************
# Title: ESA18_Workshop_AusPlots_P1.R
# Author: Bernardo Blanco-Martin, TERN
# Date: 6/10/2018
#****************************************************************************************

# This document contains the Landscape Assessment (AusCover) section of TERN's workshop at 
# the Ecological Society of Australia 2018 conference. It is based on Peter Scarth's 
# tutorial "Simple Change Detection in Raster using TERN web services and Google Earth Engine". 
# Although it (more or less) follows Dr. Scarth's tutorial steps, it has some variation in 
# its content. Dr. Scarth's tutorial uses a Jupyter Notebook with code in Python. In the 
# workshop we will use R. 

# To analyse changes in Normalized Difference Vegetation Index (NDVI) over time, we will 
# use datasets containing composited seasonal surface reflectance images (4 seasons per 
# year). The datasets were created from full time series of Landsat TM/ETM+ imagery. 
# The imagery has been composited over a season to produce new imagery that is representative 
# of the period. In this process, techniques that reduce contamination by cloud and other 
# problems were used.

# In the final part of this section of the workshop we explore the causes of NDVI change 
# in the area of most noticeable greening. To do so, a time series of the green fraction 
# is plotted. The green fraction is one of the bands in the fractional cover product. 
# This is another AusCover product hosted by TERN.
...
# found here:https://cran.r-project.org/mirrors.html. Currently commented out because it 
# libraries are preisntallled for the workshop.
...
# ---------------------------------------------------------------------------------------
# We start by: loading the required libraries, setting up a working directory, and 
# cleaning up memory.  
...
#========================================================================================
# Next, we download the file containing composited seasonal surface reflectance images 
# for Australia. The file contains 6 layers corresponding to different satellite bands. 
# In this case, we need to load each band individually. Typing the name of the raster 
# object will provide it a summary of its features (dimensions, resolution, extent, 
...
# Subsetting rasters
#========================================================================================
# The raster objects we have created contain seasonal surface reflectance images for the 
# whole of Australia. They are very large objects, which makes their manipulation and 
# processing slow. We now subset these objects to create smaller objects for our area and 
# bands of interest.
...
#========================================================================================
# We now visualise the composite image of the 3 raster bands. To do so we will follow 
# these steps:
...
# combine `rasterLayer` objects into a `rasterBrick` object. See TERN DSDP **\"Using Raster 
# Data in R\"** tutorial for more information on the raster objects classes in R. 
#  3. Plot the resulting rasterBrick using the `plotRGB` function in the `raster` package 
# and the `ggRGB` function in the `RStoolbox` library with different parameters. The latter 
# function uses the `ggplot2` graphic library framework, so it is compatible qqplots2 features. 
# That is, ggplot2 features can be added to ggRGB plots to enhance them.
...
#========================================================================================
# We will use the functions in the package `ggmap` to query map servers for a map and 
# plot the map. The function `get_map` in this package can query Google Maps, Naver Maps, 
# OpenStreeMap, and Stamen Maps. Unfortunately, since July 2018 to download maps from 
# Google Maps and Naver Maps an Application Program Interface (API) Key is required and 
# there is an associated download costs. However, we can still freely download maps from 
# Stamen. 

# The function `ggmap` from the `ggmap` package plots the raster objects produced by 
# `get_map`. `ggmap` is also built 'on top' the `ggplot2` graphic library framework, 
# so can use this library features. 

# `ggmap` retrieves maps in geographic coordinates (latitude/longitude) on the World 
# geodetic System of 1984 (WGS84) datum. Therefore, to compare our satellite images to 
# these maps we first need to reprojected to this Coordinate Reference System (CRS). 

# CRS can be stored in many different formats. The most common ones include: PROJ.4 
# (the default output from many spatial data R packages; e.g. `sp`, `rgdal`, `raster`); 
# EPSG codes, (a more concise format, found for example in the package `sf`); and 
# Well Known Text (WKT) format. The EPSG format for geographic coordinates on the WGS84 
# datum is EPSG:4326. For more details on CRS and their formats, see TERN DSDP "Using 
# Raster Data in R" tutorial.

# We use `grid.arrange` from the package `gridExtra` place multiple 'grobs' (grid 
# graphical objects, in our case ggplot2 derived graphics) in a page.
